...
class FileMoveCustom(object):
    ...
    def scheduler_function():
        try:
            ...
            FileMoveCustom.check_release = check_release
            ...
            if source_path != '' and download_path != '' and etc_path != '' and move_type != '':
                ...
                FileMoveCustom.init(source_path, download_path, etc_path)
                if move_type == '1':
                    FileMoveCustom.file_move_folder(source_path, download_path, etc_path)
                elif move_type == '2':
                    FileMoveCustom.file_move_day(source_path, download_path, etc_path)
                #파일 처리 완료후 FAKE 파일 있는지 검사
                if fake_process == 'True':
                    FileMoveCustom.init_torrent()
                    FileMoveCustom.retry_fake_file_download()
            ...
            if mvBool:
                ...
                if os.path.isfile(FILE_PATH+file):
                    ...
                    if os.path.isdir(ROOT_PATH+fileName):
                        ...
                        FileMoveCustom.move_file(FILE_PATH+file, directory+'/'+file)
                        logger.debug("### file_move_folder file complete ###")
                    else:
                        #etc 이동
                        FileMoveCustom.move_file(FILE_PATH+file, ETC_PATH+file)
                #폴더처리 시작
                    ...
                    #while start
                    while True:
                        filepath = FileMoveCustom.get_lastfile(FILE_PATH+file)
                        ...
                        if file_name.upper().find(file.upper()) < 0:
                            ...
                            if os.path.isdir(ROOT_PATH+fileName):
                                ...
                                logger.debug("move_file_name : %s", file)
                                logger.debug("move_path : %s", directory)
                                FileMoveCustom.move_file(filepath, directory+'/'+file_name)
                    ...
                    if not delete_path == '':
                        logger.debug("delete_path : %s", delete_path)
                        FileMoveCustom.remove_dir(delete_path)
                #폴더 처리 완료
    ...
    def file_move_day(source_path, download_path, etc_path):
        ...
        week = "("+FileMoveCustom.get_whichday(nowDate)+")"
        ...
        for file in fileList:
            ...
            if mvBool:
                ...
                if os.path.isfile(FILE_PATH+file):
                    ...
                    if len(fileDate) == 6:
                        #날짜에 해당하는 폴더 있으면 이동
                        checkWeek = "("+FileMoveCustom.get_whichday(fileDate)+")"
                    ...
                    if moveDir != '':
                        ...
                        FileMoveCustom.move_file(FILE_PATH+file, ROOT_PATH+moveDir+'/'+file)
                    else:
                        #etc로 이동
                ...
                #폴더처리 시작
                    ...
                    while True:
                        ...
                        if moveDir != '':
                            ...
                            logger.debug("move_path : %s", moveDir)
                            FileMoveCustom.move_file(filepath, ROOT_PATH+moveDir+'/'+file_name)
                        else:
                            #etc로 이동
                            logger.debug("target : %s", filepath)
                            logger.debug("result : %s", ETC_PATH+file_name)
                            FileMoveCustom.move_file(filepath, ETC_PATH+file_name)
    ...
    def move_file(source, target):
        ...
        if os.path.isfile(source):
            try:
                #릴검사 ON 인 경우
                if FileMoveCustom.check_release == 'True':
                ...
                target = target.replace(file_name, after_name, 1)
            finally:
                #파일 이동
                logger.debug("source name : %s, target : %s", source, target)
